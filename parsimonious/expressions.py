...
class Expression(StrAndRepr):
    ...
    def __init__(self, kind=''):
        if kind is '':
            kind = "__" + self.__class__.__name__ + "__"
        self.kind = kind
    ...
    def _match(self, text, pos, cache, error):
        ...
        if node is None and pos >= error.pos and (
                self.kind or getattr(error.expr, 'kind', None) is None):
    ...
    def as_rule(self):
        ...
        Return unicode. If I have no ``kind``, omit the left-hand side.

        """
        return (('%s = %s' % (self.kind, self._as_rhs())) if self.kind else
                self._as_rhs())
    ...
    def _unicode_members(self):
        ...
        input rule."""
        return [(m.kind or m._as_rhs()) for m in self.members]
...
class Epsilon(Expression):
    ...
    def _uncached_match(self, text, pos, cache, error):
        return Node(self.kind, text, pos, pos)
...
class Literal(Expression):
    ...
    def __init__(self, literal, kind=''):
        super(Literal, self).__init__(kind)
        self.literal = literal

    def _uncached_match(self, text, pos, cache, error):
        if text.startswith(self.literal, pos):
            return Node(self.kind, text, pos, pos + len(self.literal))
...
class Regex(Expression):
    ...
    def __init__(self, pattern, kind='', ignore_case=False, locale=False,
                 multiline=False, dot_all=False, str=False, verbose=False):
        super(Regex, self).__init__(kind)
    ...
    def _uncached_match(self, text, pos, cache, error):
        ...
        if m is not None:
            span = m.span()
            node = RegexNode(self.kind, text, pos, pos + span[1] - span[0])
...
class _Compound(Expression):
    ...
    def __init__(self, *members, **kwargs):
        ...
        super(_Compound, self).__init__(kwargs.get('kind', ''))
...
class Sequence(_Compound):
    ...
    def _uncached_match(self, text, pos, cache, error):
        ...
        return Node(self.kind, text, pos, pos + length_of_sequence, children)
    ...
    def _uncached_match(self, text, pos, cache, error):
        for m in self.members:
            ...
            if node is not None:
                ...
                return Node(self.kind, text, pos, node.end, children=[node])
...
class Optional(_Compound):
    ...
    def _uncached_match(self, text, pos, cache, error):
        node = self.members[0]._match(text, pos, cache, error)
        return (Node(self.kind, text, pos, pos) if node is None else
                Node(self.kind, text, pos, node.end, children=[node]))
    ...
    def _uncached_match(self, text, pos, cache, error):
        ...
        while True:
            ...
            if node is None or not (node.end - node.start):
                ...
                return Node(self.kind, text, pos, new_pos, children)
            children.append(node)
...
class OneOrMore(_Compound):
    ...
    def __init__(self, member, kind='', min=1):
        super(OneOrMore, self).__init__(member, kind=kind)
        self.min = min

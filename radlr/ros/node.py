...
def gennode(visitor, node, acc):
    ...
    pub_call = out_fill = set_pub = out_struct_def = out_msg_includes = ''
    for pub in node['PUBLISHES']:
        ...
        pub_call = sbl(2, pub_call, _template_pub_call.format(**d))
        out_fill = sbl(2, out_fill, _template_out_fill.format(**d))
        set_pub = sbl(2, set_pub, _template_set_pub.format(**d))
        out_msg_includes = sbl(2, out_msg_includes, _template_out_msg_include.format(**d))
        out_struct_def = sbl(2, out_struct_def, _template_out_struct_def.format(**d))
    #Over the subscribtions
    in_fill = set_sub = in_struct_def = in_msg_includes = ''
    for sub in node['SUBSCRIBED']:
        ...
        for field in sub['TOPIC']['FIELDS']:
            init_msg_fill = sbl(4, init_msg_fill,
                                _template_init_msg_fill.format(
                                                    fieldname=field._name,
                                                    fieldval=field._val,
                                                    **d))
        d['init_msg_fill'] = init_msg_fill
        in_fill = sbl(2, in_fill, _template_in_fill.format(**d))
        set_sub = sbl(2, set_sub, _template_set_sub.format(**d))
        in_msg_includes = sbl(2, in_msg_includes, _template_in_msg_include.format(**d))
        in_struct_def = sbl(2, in_struct_def, _template_in_struct_def.format(**d))
    #generate files
